import Image from "next/image";
import { Link } from "@/i18n/navigation";
import { getTranslations } from "next-intl/server";

export async function Footer() {
  const t = await getTranslations("footer");

  const serviceLinks = [
    { href: "/services/mobile-app-development/", label: t("serviceLinks.mobile") },
    { href: "/services/business-app-development/", label: t("serviceLinks.business") },
    { href: "/services/web-app-development/", label: t("serviceLinks.web") },
    { href: "/services/saas-development/", label: t("serviceLinks.saas") },
  ];

  const workLinks = [
    { href: "/work/vendrex-pos/", label: t("workLinks.vendrex") },
    { href: "/work/tally-cart/", label: t("workLinks.tallyCart") },
    { href: "/work/solar-align/", label: t("workLinks.solarAlign") },
    { href: "/work/receet-pro/", label: t("workLinks.receetPro") },
    { href: "/work/", label: t("workLinks.viewAll") },
  ];

  const companyLinks = [
    { href: "/about/", label: t("companyLinks.about") },
    { href: "/blog/", label: t("companyLinks.blog") },
    { href: "/contact/", label: t("companyLinks.contact") },
  ];

  return (
    <footer className="border-t border-border/60 bg-surface">
      <div className="mx-auto max-w-7xl px-6 py-16 lg:px-8">
        <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-4">
          <div>
            <Link href="/" className="group inline-block transition-transform duration-200 hover:scale-[1.02]">
              <Image
                src="/images/logo/boyo-apps-logo.webp"
                alt="Boyo Apps"
                width={732}
                height={243}
                className="h-12 w-auto"
              />
            </Link>
            <p className="mt-4 max-w-xs text-sm leading-relaxed text-muted">
              {t("tagline")}
            </p>
          </div>

          <div>
            <h3 className="mb-4 text-xs font-bold tracking-widest text-primary/60 uppercase">
              {t("servicesHeading")}
            </h3>
            <ul className="space-y-3">
              {serviceLinks.map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="text-sm text-muted transition-colors duration-200 hover:text-accent"
                  >
                    {link.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          <div>
            <h3 className="mb-4 text-xs font-bold tracking-widest text-primary/60 uppercase">
              {t("workHeading")}
            </h3>
            <ul className="space-y-3">
              {workLinks.map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="text-sm text-muted transition-colors duration-200 hover:text-accent"
                  >
                    {link.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          <div>
            <h3 className="mb-4 text-xs font-bold tracking-widest text-primary/60 uppercase">
              {t("companyHeading")}
            </h3>
            <ul className="space-y-3">
              {companyLinks.map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="text-sm text-muted transition-colors duration-200 hover:text-accent"
                  >
                    {link.label}
                  </Link>
                </li>
              ))}
            </ul>
            <h3 className="mb-4 mt-8 text-xs font-bold tracking-widest text-primary/60 uppercase">
              {t("connectHeading")}
            </h3>
            <ul className="space-y-3">
              <li>
                <a
                  href="mailto:boyoapps@gmail.com"
                  className="inline-flex items-center gap-1.5 text-sm text-muted transition-colors duration-200 hover:text-accent"
                >
                  <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z" />
                    <polyline points="22,6 12,13 2,6" />
                  </svg>
                  {t("social.email")}
                </a>
              </li>
              <li>
                <a
                  href="https://www.linkedin.com/in/steeve-st-fleur/"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1.5 text-sm text-muted transition-colors duration-200 hover:text-accent"
                >
                  <svg className="h-4 w-4" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
                  </svg>
                  {t("social.linkedin")}
                </a>
              </li>
              <li>
                <a
                  href="https://github.com/stfleurs"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1.5 text-sm text-muted transition-colors duration-200 hover:text-accent"
                >
                  <svg className="h-4 w-4" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
                  </svg>
                  {t("social.github")}
                </a>
              </li>
              <li>
                <Link
                  href="/contact/"
                  className="inline-flex items-center gap-1.5 text-sm text-muted transition-colors duration-200 hover:text-accent"
                >
                  <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z" />
                    <polyline points="22,6 12,13 2,6" />
                  </svg>
                  {t("social.sendMessage")}
                </Link>
              </li>
            </ul>
          </div>
        </div>

        <div className="mt-16 flex flex-col items-center justify-between gap-4 border-t border-border/60 pt-8 sm:flex-row">
          <p className="text-xs text-muted">
            {t("copyright", { year: new Date().getFullYear() })}
          </p>
          <div className="flex gap-6">
            <Link href="/privacy/" className="text-xs text-muted transition-colors duration-200 hover:text-accent">
              {t("privacyPolicy")}
            </Link>
            <Link href="/terms/" className="text-xs text-muted transition-colors duration-200 hover:text-accent">
              {t("terms")}
            </Link>
          </div>
        </div>
      </div>
    </footer>
  );
}
